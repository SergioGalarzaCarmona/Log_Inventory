    ...
    if request.method == "GET":
        return render(
            ...
            "Objects/main.html",
            {
                ...
                "object_group_form": ObjectsGroupForm(
                    user=profile.user if type == "profile" else profile.profile.user,
                    instance=None,
                ),
            },
        ...
        if object:
            ...
            if not form.is_valid():
                messages.error(
                    request, "Hubo un error al tratar de crear el grupo de objetos."
                )
                return render(
                    request,
                    "Objects/main.html",
    ...
    if request.method == "GET":
        ...
        if delete_image:
            ...
            if image_before == "default_object.jpg":
                messages.error(
                    request,
                    "No se pudo borrar la imagen ya que el objeto no tenía una imagen relacionada.",
                )
        ...
        form = ObjectForm(
            ...
            user=profile.user if type == "profile" else profile.profile.user,
            instance=object,
            initial=object.__dict__,
        )
        if not form.has_changed():
            messages.warning(
                request,
                "No se realizaron cambios porque no había ningun campo editado.",
            )
            return render(
                request,
                "Objects/object.html",
...
def manage_object_groups(request):
    ...
    if request.method == "GET":
        ...
        if request.POST.get("delete_image", False):
            ...
            if not form.is_valid():
                messages.error(
                    request, "Hubo un error en los datos ingresados del usuario."
                )
                return render(
                    request,
                    "Objects/object_groups.html",
...
def log(request):
    ...
    query_users = UserChanges.objects.filter(main_user=profile_admin.user).order_by(
        "-date"
    )
    query_user_groups = GroupChanges.objects.filter(
        main_user=profile_admin.user
    ).order_by("-date")
    query_objects = Transaction.objects.filter(user=profile_admin.user).order_by(
        "-date"
    )
    query_object_groups = GroupObjectsChanges.objects.filter(
        main_user=profile_admin.user
    ).order_by("-date")
    ...
    if request.method == "GET":
        ...
        if not form.is_valid():
            messages.error(
                request, "Hubo un error al tratar de exportar el inventario."
            )
            return render(
                request,
                "Objects/log.html",
        ...
        response["Content-Disposition"] = (
            'attachment; filename="Log_Inventory_Report.xlsx"'
        )

...
class PermissionsGroup(models.Model):
    permissions_id = models.ManyToManyField(Permissions, related_name="permissions")
...
class SubprofilesGroup(BaseAuditModel):
    ...
    image = models.ImageField(
        default="default_group.jpg", upload_to="subprofiles_groups_images"
    )
...
class Subprofile(BaseAuditModel):
    ...
    profile = models.ForeignKey(Profile, on_delete=models.CASCADE)
    image = models.ImageField(default="default.jpg", upload_to="profile_images")
    group = models.ForeignKey(SubprofilesGroup, on_delete=models.CASCADE)
...
class UserChanges(models.Model):
    ...
    user_changed = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_changed"
    )
...
class GroupChanges(models.Model):
    main_user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="main_user_group"
    )
    group_changed = models.ForeignKey(
        SubprofilesGroup, on_delete=models.CASCADE, related_name="user_changed_group"
    )
